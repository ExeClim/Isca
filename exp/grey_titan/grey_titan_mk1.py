...
#     },
    ...
    'constants_nml': { #Titan Values
        'orbital_period': 928523294.,
        'solar_const':15.078, 
        'radius':2575.0e3,
        'rdgas':188.7,
        'kappa':0.172,
        'rotation_period':1377631.,
    },
    ...
    for conv in conv_schemes:
        for depth_val in depths:
            for per_value in pers:
                exp = Experiment('grey_titan_12_year_1_stephen_mod_2_high_solar_abs', codebase=cb) #'grey_mars_titan_solarconst_mk36_per_value'+str((per_value))+'_'+conv+'_mld_'+str(depth_val), codebase=cb)
                exp.clear_rundir()
                exp.diag_table = diag
                exp.namelist = namelist.copy()
                exp.namelist['constants_nml']['grav']     = scale * 1.352
