...
from plevel_fn import plevel_call, daily_average, join_files, two_daily_average
import sys
import os
import time
import pdb
...
#exp_name_list=['simple_continents_post_princeton_qflux_anoms_'+str(x) for x in range(1,11)]
...
level_set='tom_diagnostics' #Default is the standard levels used previously. ssw_diagnostics are the ones blanca requested for MiMa validation
...
if level_set=='standard':
	...
	plevs['6hourly']=' -p "25000 50000 85000 92500"'
	plevs['daily']  =' -p "25000 50000 85000 92500"'
	
	var_names['monthly']='-a slp'
	var_names['timestep']='-a'
	var_names['6hourly']='ucomp slp height vor t_surf vcomp omega'
	var_names['daily']='ucomp slp height vor t_surf vcomp omega'
	file_suffix='_interp_new_model_lev'
...
for exp_name in exp_name_list:
	for n in range(nfiles):
		for avg_or_daily in avg_or_daily_list:
			...
			nc_file_out = base_dir+'/'+exp_name+'/run'+number_prefix+str(n+start_file)+'/atmos_'+avg_or_daily+file_suffix+'.nc'
