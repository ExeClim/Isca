...
diag = DiagTable()
diag.add_file('atmos_10period', 40, 'minutes', time_units='minutes')
...
diag.add_field('dynamics', 'press_full', time_avg=True)
...
if __name__ == "__main__":
    ...
    albedo_list = [0.1]
    pressures_bar = [1]   # in bar
    lw_optical_depths = {1:1.6}  # mapping Ps -> tau
    ...
    for Ps in pressures_bar:
        for a in albedo_list:
            ...
            run_exp = exp.derive(
                f"TOI_561b/Ab_0/Ps_exp/TOI_561b_H2O_atm_lw_{od_lw}_{Ps}_bar"
            )
