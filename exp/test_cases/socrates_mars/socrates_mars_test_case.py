...
# NOTE: this test case requires the Mars-specific Socrates spectral files below to be present
# in the `input` folder next to this script:
#   sp_lw_17_dsa_mars, sp_lw_17_dsa_mars_k, sp_sw_42_dsa_mars_sun, sp_sw_42_dsa_mars_sun_k
...
exp = Experiment('socrates_mars_test_experiment', codebase=cb)
...
#values, taken from exp/socrates_mars/socrates_mars.py, which only had a single combination
#(no convection scheme, 2 m mixed-layer depth, per=70.85 degrees), reused unchanged here.
