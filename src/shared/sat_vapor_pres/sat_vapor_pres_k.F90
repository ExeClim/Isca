...
! benefit and could be done as a second step anyway, if necessary.
 ...
 subroutine sat_vapor_pres_init_k(table_size, tcmin, tcmax, TFREEZE, TFREEZE_WATER, HLV, RVGAS, ES0, triple_point_pres, err_msg, &
...
! This routine has been generalized to return tables for any temperature range and resolution
 ...
 real, intent(in) :: TFREEZE, TFREEZE_WATER, HLV, RVGAS, ES0, triple_point_pres
...
! increment used to generate derivative table
  ...
  integer :: i
   ...
   if (construct_table_wrt_liq_and_ice) then
      ...
      tminl = real(tcmin)+TFREEZE_WATER  ! minimum valid temp in table. Always use TFREEZE_WATER here, as this is just converting tcmin from celcius to kelvin.
      dtinvl = 1./dtres
      tepsl = .5*dtres
      tinrc = .1*dtres
