...
if __name__ == "__main__":
    ...
    S_list = [40169874.77992497,  5991222.75246139,  2819455.49151964,1747087.85955473,  1229373.01280298,   931122.8405196 ,739935.41992842]
    P_list = [0.1, 0.41666667, 0.73333333, 1.05, 1.36666667,1.68333333, 2.]
    d_list = [6.30817489e+08, 1.63341384e+09, 2.38106550e+09, 3.02479949e+09,3.60588711e+09, 4.14333906e+09, 4.64790300e+09]

    od_list = [4.7,7.8,15.7]
    M = 2.016
    Rd = 8.314 / M * 1e3
    Cp = 17139
    gamma = Rd / Cp
    ...
    for i in range(len(S_list)):
        ...
        P = P_list[i]
        D = d_list[i]
        for j in range(len(od_list)):
            od_lw = od_list[j]   # get optical depth for given Ps
            run_exp = exp.derive(
                f"TOI_561b/OD_exp/H2/TOI_561b_H2_atm_lw_P_{P:0.4f}_od_{od_lw}_5"
            )
            ...
            run_exp.namelist['constants_nml']['omega'] = 2*np.pi/(P*86400)
            ...
            run_exp.namelist['two_stream_gray_rad_nml']['solar_constant']=S
            run_exp.namelist['two_stream_gray_rad_nml']['d_stellar']=D
            ...
            run_exp.namelist['diffusivity_nml']['tj_bl_pres'] = 85000. * 5
            run_exp.namelist['diffusivity_nml']['tj_strato_pres'] = 10000. / 2. * 5
            run_exp.namelist['damping_driver_nml']['sponge_pbottom'] = 260. * 5
