...
real    :: rich_crit  = 2.0 !s critical richardson number above which drag coeffs are set to drag_min - see eq 37 of monin_obukhov.tech.ps
real    :: drag_min   = 1.e-05 !s minimum drag
real    :: relax_time = 0.            
logical :: neutral    = .false. !s if true we just make all the profiles neutral and logarithmic.
integer :: stable_option  = 1 !s two version for the similarity functions on the stable side, as described in section 3.2 of monin_obukhov.tech.ps
real    :: zeta_trans     = 0.5 ! if stable_option = 2 there is a transition value of zeta between two functions - see eqs 38-39 of monin_obukhov.tech.ps
...
if(neutral) then !s if neutral=.true. then we just make all the profiles neutral and logarithmic.
  ...
  where(avail)
    ...
    bs   = vonkarm/fh
    qs   = vonkarm/fq
    drag_m    = us*us !s as in Frierson's thesis equation 2.19.
    drag_t    = us*bs
    drag_q    = us*qs
    u_star = us*speed
    b_star = bs*delta_b
...
! stabl and unstabl (which have size npts and nptu) based on the sign of rich
  call separate_stabilities(npts, nptu, stabl, unstabl, rich, avail, r_crit) !s Here it's deciding where the unstable and stable points are
! unstable points -- now treated as the neutral case.  
!s Frierson's thesis, section 2.4.2, describes their simplification to MOS, which treats unstable points as neutral
  if(nptu > 0) then
...
!      fq(unstabl(i)) = fq_1d(i)
       fm(unstabl(i))   = log(z(unstabl(i))/z0(unstabl(i))) !s this is the neutral `law of the wall' logarithmic solution
...
! no iteration is necessary -- we have an analytic solution
  if(npts > 0) then !s stable points solved analytically now, as in Frierson's thesis section 2.4.2. 
!    do i =1, npts
...
!   end where
     ...
     where (avail)
       drag_m = drag_m_eq !s here is where drag_m=drag_m_eq for whatever reason.
       drag_t = drag_t_eq
       drag_q = drag_q_eq
       u_star = us*speed
       b_star = bs*delta_b
