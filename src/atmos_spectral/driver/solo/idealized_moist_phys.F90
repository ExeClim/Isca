...
if(mixed_layer_bc) then
  ...
  call mixed_layer_init(is, ie, js, je, num_levels, t_surf, bucket_depth, get_axis_id(), &
           Time, albedo, rad_lonb_2d(:,:), rad_latb_2d(:,:), land, bucket, h_ice, a_ice,t_ml )
...
if(two_stream_gray) then
   call two_stream_gray_rad_down(is, js, Time, &
                       ...
                       net_surf_sw_down(:,:),  &
                       surf_lw_down(:,:), albedo, &
                       grid_tracers(:,:,:,previous,nsphum)) !!!!! Chung add a_ice ? No
end if
...
if(mixed_layer_bc)  call mixed_layer_end(t_surf, bucket_depth, bucket ,h_ice,a_ice, t_ml) ! thermodynamic ice Chung
