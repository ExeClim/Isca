# simulate Trappist-1b with the socrates radiation scheme
# stellar spectra
# radius: 1.116 R_earth
# mass: 1.374 M_earth
# surface gravity: 1.103 g_earth
# orbital period: 1.5 days
# tidally locked
# stellar constant: 4.164 S0
...
exp = Experiment('Trappist1btest5', codebase=cb)
exp.clear_rundir()
...
exp.namelist = namelist = Namelist({
    ...
    'socrates_rad_nml': {
        ...
        'lw_spectral_filename':os.path.join(GFDL_BASE,'src/atmos_param/socrates/src/trunk/data/spectra/sp_test/sp_lw_b72_Trappist1'), #check _k used in the file
        'sw_spectral_filename':os.path.join(GFDL_BASE,'src/atmos_param/socrates/src/trunk/data/spectra/sp_test/sp_sw_b72_Trappist1'),
        'dt_rad':3600, # P=1.5 day dtrad=1day???
...
if __name__=="__main__":
    cb.compile()
    exp.run(87, use_restart=True, num_cores=NCORES, overwrite_data=True)
    overwrite = False
    for i in range(88,181):
