...
RESOLUTION = 'T42', 60 #T42 horizontal resolution, 25 levels in pressure
...
namelist = Namelist({
    ...
    'damping_driver_nml': {
        ...
        'sponge_pbottom':  112.05,           #Bottom of the model's sponge down to 1.1205 for 3 model layers for Titan (units are Pa)
    },
...
#     },
    ...
    'fms_nml': {
        'domains_stack_size': 61440 #Setting size of stack available to model, which needs to be higher than the default when running at high spatial resolution
    },
    ...
    for conv in conv_schemes:
        for depth_val in depths:
            for per_value in pers:
                exp = Experiment('grey_titan_T42_daily_tapio_rad_with_sponge_mk10_tau_diag_60_levels', codebase=cb) #name of folder in which .nc files are output
                exp.clear_rundir()
