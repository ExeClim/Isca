...
exp = Experiment('MiMA_MLD60_ThermoIce_coalboff_alb022045_86400_T42z30_4xCO2_rs', codebase=cb)
...
#diag.add_file('atmos_penta'  ,  5, 'days', time_units='days')
#diag.add_file('atmos_3hours' ,  3, 'hours', time_units='hours')
...
diag.add_field('dynamics', 'height', time_avg=True) # 2022/10/27
diag.add_field('dynamics', 'height_half', time_avg=True) # 2022/10/27
...
diag.add_field('dynamics', 'ucomp_height', time_avg=True) # 2022/10/27
diag.add_field('dynamics', 'vcomp_height', time_avg=True) # 2022/10/27
diag.add_field('dynamics', 'omega_height', time_avg=True) # 2022/10/27
diag.add_field('dynamics', 'sphum_u', time_avg=True) ### 2022/11/05
diag.add_field('dynamics', 'sphum_v', time_avg=True) ### 2022/11/05
diag.add_field('dynamics', 'sphum_w', time_avg=True) ### 2022/11/05
...
#
diag.add_field('rrtm_radiation', 'coszen', time_avg=True) ### output for offfline kernels
###########
exp.diag_table = diag
...
#        'albedo_value': 0.1,
        ...
        'do_thickness_lock':False,        ### Chung:active thermodynamic ice free run
...
#        'ozone_file':'ozone_1990',
   ...
   #     'del_coalb_toa_P2': 0.0
    ...
    'spectral_dynamics_nml': {
        ...
        'initial_sphum':[2.e-6],
        'vert_coord_option':'even_sigma', ### use the vertical levels from CESM resolution: T42z30_T42_mg17
        'surf_res':0.5,
        'scale_heights' : 11.0,
        'exponent':7.0,
        'robert_coeff':0.03
...
#,
#    'vert_coordinate_nml': {
#        'bk':[ 0.00999999974737875, 33.3333015441895, 66.6666999459267, 100.000001490116, 133.332997560501, 166.666999459267, 200.000002980232, 233.333006501198, 266.667008399963, 300.000011920929, 333.332985639572, 366.667002439499, 400.000005960464, 433.33300948143, 466.666996479034, 500, 533.333003520966, 566.667020320892, 600.000023841858, 633.333027362823, 666.666984558105, 699.999988079071, 733.332991600037, 766.667008399963, 800.000011920929, 833.333015441895, 866.666972637177, 899.999976158142, 933.332979679108, 966.666996479034, 1000 ], # use ilev in CESM (30 levels, T42z30_T42_mg17)
...
if __name__=="__main__":
    exp.run(638, use_restart=True , num_cores=NCORES ,overwrite_data=True )
#    exp.run(1, use_restart=True , restart_file='/data/groups/feldl/pochung/Isca_out/MiMA_MLD60_ThermoIce_coalboff_alb022045_86400_T42z30_4xCO2_rs/CTL_res0840.tar.gz' , num_cores=NCORES ,overwrite_data=True )
    for i in range(639,841):
