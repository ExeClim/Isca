...
#lonb_naive=np.zeros(lons.shape[0]+1)

#for tick in np.arange(1,lons.shape[0]):
#	lonb_naive[tick]=(lons[tick-1]+lons[tick])/2.

lons = [0.]
lonb_naive = [0., 360.]

latb_naive=np.zeros(lats.shape[0]+1)

for tick in np.arange(1,lats.shape[0]):
	latb_naive[tick]=(lats[tick-1]+lats[tick])/2.
...
#p_half = resolution_file.variables['phalf'][:]
...
p_half_naive=np.zeros(p_full.shape[0]+1)

p_half_naive[0]=1200.

for tick in np.arange(1,p_full.shape[0]):

	p_half_naive[tick]=(p_full[tick-1]+p_full[tick])/2.


p_half=p_half_naive

#p_half[0]=0.
#p_half_expanded=np.insert(p_half,0,[0., 0.046817, 0.115544])
#p_full_expanded=np.insert(p_full,0,[0.017223, 0.078654, 0.180749])
#ozone_expanded= np.insert(ozone_in,0,[ozone_in[:,0,:,:],ozone_in[:,0,:,:],ozone_in[:,0,:,:]],1)
#pdb.set_trace()


#p_half = p_half_expanded
#p_full = p_full_expanded

#ozone_in = ozone_expanded
...
co2_file = Dataset('ozone_1990_cmip5.nc', 'w', format='NETCDF3_CLASSIC')
...
times.climatology = '1990-01-01 00:00:00, 1999-12-31 00:00:00'

co2_array_netcdf = co2_file.createVariable('ozone_1990_cmip5','f4',('time','pfull', 'lat','lon',))
...
times[:]     = time_arr #date2num(time_arr,units='days since 0001-01-01 00:00:00.0',calendar='360_day')

co2_array_netcdf[:] = ozone_new[:,::-1,:]
