...
inputfiles = [os.path.join(GFDL_BASE,'input/rrtm_input_files/ozone_1990.nc'), os.path.join(base_dir,'input/sst_clim_amip.nc'), 
              os.path.join(GFDL_BASE,'input/land_masks/era_land_t42.nc'), os.path.join(base_dir,'input/siconc_clim_amip.nc')]

#sc_diag_names = ['eis_rh']
#sc_diag_names = ['eis_wood']
#sc_diag_names = ['cam', 'eis_wood_rh', 'eis_wood', 'ectei', 'ectei_rh', 'slingo', 'dthdp', 'eis_rh', 'slingo_no_rh'] #'eis_wood_rh_base_gt_val'] #'ectei']#, 'eis_rh'] #'ectei_rh']#
#sc_diag_names = [ 'dthdp', 'eis_rh', 'slingo_no_rh'] #'eis_wood_rh_base_gt_val'] #'ectei']#, 'eis_rh'] #'ectei_rh']#
land_hum_pref_arr = [0.8, 0.9, 0.95, 0.6, 1]
for land_hum_pref in land_hum_pref_arr:
    #sc_diag_names = ['Park_ELF'] #'eis_wood_rh_base_gt_val'] #'ectei']#, 'eis_rh'] #'ectei_rh']#
    #for sc_diag_name in sc_diag_names:
    sc_diag_name = 'Park_ELF' #'eis_wood_rh_base_gt_val'] #'ectei']#, 'eis_rh'] #'ectei_rh']#
    print(sc_diag_name)
    exp = Experiment('soc_test_amip_linear_qcl_with_temp_sc_'+sc_diag_name+'_new_lev_land_hum_pref_'+str(land_hum_pref), codebase=cb)
    exp.clear_rundir()
    ...
    coeff_fn = '/home/links/ql260/Documents/Exps_Analysis/clouds/data_new_lev/paras_a_b.npy'
    ...
    exp.namelist = namelist = Namelist({
        ...
        'surface_flux_nml': {
            ...
            'do_simple': True,
            'old_dtaudv': True,
            'land_humidity_prefactor': land_hum_pref,
        },
