...
exp = Experiment('55_Cnc_e_Ps_exp_1', codebase=cb)
...
#     },
    ...
    'two_stream_gray_rad_nml': {
        ...
        'ir_tau_pole':1,#1.5, 
        'del_sol':1.2, 
        'solar_constant':2667*1361, 
        'R_stellar':0.943*6.957e8,
        'd_stellar':0.015436*1.496e11,
        'linear_tau':1.,#0.2, 
        'odp':1.0
    },
    ...
    'constants_nml':{
        'omega': 2*np.pi / (0.7365474*86400.),#7.2921150e-5 / 8., 
        'es0': 1.e-6,
        'rdgas':8.314/28.0134*1e3,
        'kappa':(8.314/28.0134*1e3)/(1285-(8.314/28.0134)*1e3),
        'radius':1.875*6.371e6,
        'grav':(6.67e-11*7.99*5.972e24)/((1.875*6.371e6)**2), 
        'pstd':10.e5, 
        'pstd_mks':10.e6,
    },
...
# create an Experiment object to handle the configuration of model parameters
# and output diagnostics
exp = Experiment('55_Cnc_e_Ps_exp', codebase=cb)
...
#     },
    ...
    'constants_nml':{
        'omega': 2*np.pi / (0.737*86400.),#7.2921150e-5 / 8., 
        'es0': 1.e-6,
        'rdgas':8.314/28.0134*1e3,
        'kappa':(8.314/28.0134*1e3)/(1285-(8.314/28.0134)*1e3),
        'radius':1.91*6.371e6,
...
if __name__ == "__main__":
    ...
    lw_tau = {1: 0.8, 3: 2.4, 5: 4, 10: 8} # τ_LW(Ps), composition-independent

    # Gas-specific properties (M in g/mol, Cp in J/(kg·K))
    gases = {
        'H2' : {'M': 2.01588, 'Cp': 17139.0},
        'N2' : {'M': 28.0134, 'Cp': 1285.0},
    }

    for gas, props in gases.items():
        M  = props['M']
        Cp = props['Cp']
        Rd = 8.314 / M * 1e3      # J/(kg·K)
        kappa = Rd / Cp

        for Ps in pressures_bar:
            od_lw = lw_tau[Ps]    # same τ for all compositions

            for a in albedo_list:
                run_exp = exp.derive(
                    f"55_CnC_e_2/{gas}/TOI_561b_{gas}_atm_lw_{od_lw}_{Ps}_bar_albedo_{a}"
                )

                Ps_Pa = Ps * 1e5

                # Update namelist for this case
                ...
                # Thermodynamics by gas
                run_exp.namelist['constants_nml']['rdgas'] = Rd
                run_exp.namelist['constants_nml']['kappa'] = kappa

                # Pressure-scaled turbulence/damping levels
                run_exp.namelist['diffusivity_nml']['tj_bl_pres']      = 85000.0 * Ps
                run_exp.namelist['diffusivity_nml']['tj_strato_pres']  = (10000.0 / 2.0) * Ps
                run_exp.namelist['damping_driver_nml']['sponge_pbottom'] = 260.0 * Ps
