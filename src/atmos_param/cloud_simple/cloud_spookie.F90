module cloud_spookie_mod
  ...
  character(len=128) :: version='$Id: cloud_spookie.F90,v 1.0 2021/05/11$'
  character(len=128) :: tag='Simple cloud scheme - SPOOKIE Protocol'
  ...
  namelist /cloud_spookie_nml/  cca_lower_limit, spookie_protocol,      &
  ...
  subroutine cloud_spookie_init (axes, Time)
    ...
    ierr=1
    do while (ierr /= 0)
        read  (unit, nml=cloud_spookie_nml, iostat=io, end=10)
       ierr = check_nml_error (io, 'cloud_spookie_nml')
    enddo
  ...
  end subroutine cloud_spookie_init

  !-----------------------------------------------

  subroutine cloud_spookie(p_half, p_full, Time,   &
                      temp, q_hum,                &
                      ! outs 
    ...
    if (do_init) call error_mesg ('cloud_spookie',  &
         'cloud_spookie_init has not been called.', FATAL)
