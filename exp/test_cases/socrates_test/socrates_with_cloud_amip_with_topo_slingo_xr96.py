...
#sc_diag_name = 'ectei'#'eis_wood' #'eis_wood_rh' #'eis_wood_rh_base_gt_val'] #'ectei']#, 'eis_rh'] #'ectei_rh']#
sc_diag_names = ['ectei', 'ectei', 'ectei_rh', 'ectei_rh']#'eis_wood' #'eis_wood_rh' #'eis_wood_rh_base_gt_val'] #'ectei']#, 'eis_rh'] #'ectei_rh']#
#cf_diag_methods = ['slingo', 'XR96']
#do_diag_sc_arr = [True, True]
#cf_diag_methods = ['slingo', 'XR96', 'slingo', 'XR96', 'slingo', 'XR96']
#do_diag_sc_arr = [False, False, True, True, True, True]
cf_diag_methods = ['slingo', 'XR96', 'slingo', 'XR96']
do_diag_sc_arr = [True, True, True, True]
for sc_diag_name, cf_diag_method, do_diag_sc in zip(sc_diag_names, cf_diag_methods, do_diag_sc_arr): 
    if do_diag_sc:
        exp = Experiment('soc_test_amip_'+cf_diag_method+'_qcl_with_temp_sc_'+sc_diag_name, codebase=cb)
    else:
        exp = Experiment('soc_test_amip_'+cf_diag_method+'_qcl_with_temp', codebase=cb)
    exp.clear_rundir()
    ...
    exp.namelist = namelist = Namelist({
        ...
        'cloud_simple_nml': {
            ...
            'rhc700': 0.7,
            'rhc200': 0.3,
            'do_simple_rhcrit': True, #False, # Not reasonable for XR96 and Slingo...
            'do_read_scm_rhcrit': False,
            'do_linear_diag': False,
            #'scm_linear_coeffs': coeff_arr, 
            #'do_qcl_two_paras': True,
            'cf_diag_formula_name': cf_diag_method,
    ...
    exp.namelist['cloud_simple_nml']['do_add_stratocumulus'] = do_diag_sc
